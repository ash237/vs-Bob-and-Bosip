...
class Um extends MusicBeatSubstate
{
	var bg:FlxSprite;
	...
	public var gallery:FlxTypedGroup<FlxSprite>;
	public var pictures:FlxTypedGroup<FlxSprite>;
	...
	var twitterLoadingLinks:Array<String> = [
		...
		'https://twitter.com/Justifited',
		'https://twitter.com/SpookyGh0sty_',
		'https://twitter.com/Franbobisdraw',
		'https://twitter.com/Franbobisdraw',
		'https://twitter.com/NabNab_222',
	...
	public var imageLocations:Array<String> = [];
	...
	public function new(s:FlxPoint, bb:FlxSprite)
	{
		...
		bb.visible = false;
		super();
		var zoom:FlxSprite = new FlxSprite(s.x, s.y).loadGraphic(Paths.image('desktop/gallery/cool/zoom'));
		zoom.origin.set(0, 0);
		...
		FlxTween.tween(zoom, {x: 0, y: 0, alpha: 1}, 0.3, {ease: FlxEase.cubeOut, onComplete: function(twn:FlxTween) {
			...
			image = new FlxSprite(10, 94).loadGraphic(Paths.image('desktop/gallery/cool/screen'));
			add(image);
			border = new FlxSprite().loadGraphic(Paths.image('desktop/gallery/cool/galleryBorder'));
			add(border);
	...
	if (isExist) {
			...
			if (inCat == 0) {
				if (FlxG.mouse.overlaps(image)) {
					image.color = FlxColor.fromHSL(image.color.hue, image.color.saturation, 1, 1);
				} else {
					image.color = FlxColor.fromHSL(image.color.hue, image.color.saturation, 0.7, 1);
				}

				if (FlxG.mouse.overlaps(xText)) {
					xText.color = FlxColor.fromHSL(xText.color.hue, xText.color.saturation, 1, 1);
				} else {
					xText.color = FlxColor.fromHSL(xText.color.hue, xText.color.saturation, 0.7, 1);
				}
				if (FlxG.mouse.justPressed && FlxG.mouse.overlaps(image) || FlxG.mouse.justPressed && FlxG.mouse.overlaps(fanmadeImage) || FlxG.mouse.justPressed && FlxG.mouse.overlaps(loadingImage)) {
						selectedGallery = gallery;
						selectedPictures = pictures;
						selectedImageLocations = imageLocations;
						selectedMenu = 'official';
					
					resetPositions();
					inCat = 99;
					isExist = false;
					FlxTween.tween(image, {alpha: 0}, 0.6, {});
				...
				if (FlxG.mouse.justPressed && FlxG.mouse.overlaps(xText) || FlxG.keys.justPressed.ESCAPE && !FlxG.mouse.justPressed) {
					...
					bb.visible = false;
					xText.visible = false;
					var zoom:FlxSprite = new FlxSprite().loadGraphic(Paths.image('desktop/gallery/cool/zoom'));
					zoom.origin.set(0, 0);
					add(zoom);
			...
			if (inCat == 1) {
				...
				if (FlxG.mouse.justPressed && FlxG.mouse.overlaps(xText) || FlxG.keys.justPressed.ESCAPE && !FlxG.mouse.justPressed) {
					...
					FlxTween.tween(image, {alpha: 1}, 0.6, {});
					new FlxTimer().start(1, function(tmr:FlxTimer) {
