...
class TitleState extends MusicBeatState
{
	...
	static var leftOnce:Bool = false;
	...
	function startIntro()
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image('Menu_BG'));
		// bg.antialiasing = true;
		// bg.setGraphicSize(Std.int(bg.width * 0.6));
		// bg.updateHitbox();
		add(bg);

		logoBl = new FlxSprite(-150, -10);
		...
		bob = new FlxSprite(FlxG.width * 0.5, -40);
		bob.scale.set(0.55, 0.55);
		bob.frames = Paths.getSparrowAtlas('Start_Screen_bob');
		bob.animation.addByPrefix('idle', 'amortitle', 24, false);
		bob.antialiasing = true;
		add(bob);

		bosip = new FlxSprite(-230, -130);
		bosip.scale.set(0.55, 0.55);
		bosip.frames = Paths.getSparrowAtlas('Start_Screen_bosip');
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			FlxTween.tween(bob, {y: bob.y + 50}, 0.6, {ease: FlxEase.quadIn, onComplete: function(twn:FlxTween) {
				FlxTween.tween(bob, {y: bob.y + 500}, 0.6, {ease: FlxEase.quadIn});
			}});

			FlxTween.tween(bosip, {y: bosip.y + 50}, 0.6, {ease: FlxEase.quadIn, onComplete: function(twn:FlxTween) {
				FlxTween.tween(bosip, {y: bosip.y + 600}, 0.6, {ease: FlxEase.quadIn});
			}});
